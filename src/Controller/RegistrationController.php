...
class RegistrationController extends AbstractController
{
    ...
    public function register(Request $request, UserPasswordHasherInterface $userPasswordHasher, UserAuthenticatorInterface $userAuthenticator, UsersAuthenticator $authenticator, EntityManagerInterface $entityManager, SendMailService $mail, JWTService $jwt): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $mail->send(
                ...
                $user->getEmail(),
                'Activation de votre compte sur le site Shelter.com',
                'register',
                [
                    'user' => $user,
                    'token' => $token
    ...
    public function verifyUser($token, JWTService $jwt, UsersRepository $usersRepository, EntityManagerInterface $em): Response
    {
        ...
        // Si les vérifications ne sont pas validées, un problème se pose dans le token
    ...
    public function resendVerif(JWTService $jwt, SendMailService $mail, UsersRepository $user): Response
    {
        $user = $this->getUser();
